...
<div class="container">
    <form method="POST" class="form-group">
        <div class="row">
            ...
            <div class="col-sm-4">
                <input type="text" name="nomeusu" value="<?php echo $usuario['nomeusu'] ;?>" size="40" maxlength="50" class="form-control" required autofocus />
            </div>
        </div>
        <div class="row">
            <div class="col-sm-3 prompt">E-mail</div>
            <div class="col-sm-4">
                <input type="email" name="email" value="<?php echo $usuario['email'] ;?>" size="80" maxlength="100" class="form-control" require'd />
            </div>
        </div>
        <div class="row">
            <div class="col-sm-3 prompt">Nome para Acesso ao Sistema</div>
            <div class="col-sm-4">
                <input type="text" name="username" value="<?php echo $usuario['username'] ;?>" size="25" maxlength="30" class="form-control" disabled />
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-sm-4">
                <select name='grupodeacesso'>
                    <?php foreach ($gruposdeacesso as $item) : ?>
                        <option value="<?php echo $item['ID'] ;?>" <?php ($usuario['accessgroup']==$item['ID']) ? "selected='selected'":"" ;?>  ><?php echo $item['NAME'] ;?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
